...
var options = { root: "data" }
  , optionsBackupOff = { root: "data", backup: false }
...
vows.describe("fsStore basic").addBatch({
  ...
  , "on file exists": {
      topic: function(store) {
        mkFile(store, "hasFile.json");
        store.has("hasFile.json", this.callback);
      }
    , "return true": function(err, flag, id) {
        assert.isNull(err);
        assert.isTrue(flag);
        assert.equal(id.charAt(0), "/");
        assert.equal(id, "/hasFile.json");
  ...
  , "on file exists": {
      topic: function(store) {
        mkFile(store, "getFile.json");
        store.get("getFile.json", this.callback);
      }
    , "load the file data": function(err, data, id) {
        assert.isNull(err);
        assert.equal(id.charAt(0), "/");
        assert.equal(id, "/getFile.json");
  ...
  , "load": {
      topic: function(store) {
        mkFile(store, "getFileEmpty.json", "");
        store.get("getFileEmpty.json", this.callback);
      }
    , "an empty file": function(err, data, id) {
        assert.isNull(err);
        assert.equal(id.charAt(0), "/");
        assert.equal(id, "/getFileEmpty.json");
        assert.equal(this, storeGET);
        assert.isEmpty(data);
      }
    }
  ...
  , "on file exists": {
      topic: function(store) {
        mkFile(store, "setFile.json");
        store.set("setFile.json", {}, this.callback);
      }
  ...
  , "ignore if dir and its backup": {
      topic: function(store) {
        ...
        mkFile(store, "setDirExistingWithBackup/setFile.json");
        mkDir(store, "setDirExistingWithBackup" + store.options.backupSuffix);
        mkFile(store, "setDirExistingWithBackup"+ store.options.backupSuffix + "/setFileBak.json");
  ...
  , "": {
      topic: function(store) {
        mkFile(store, "removeFile.json");
        store.remove("removeFile.json", this.callback);
      }
    , "remove the file": function(err, id) {
        assert.isNull(err);
        assert.equal(id.charAt(0), "/");
        assert.equal(id, "/removeFile.json");
  ...
  , "on file exists": {
      topic: function(store) {
        mkFile(store, "removeFileWithBackup.json");
        store.remove("removeFileWithBackup.json", this.callback);
      }
    , "make a backup": function(err, id) {
  ...
  , "remove a non empty directory": {
      topic: function() {
        ...
        mkFile(storeREMOVE, "removeDirNonEmpty/somedata.json");
  ...
  , "remove a directory with backup": {
      topic: function() {
        ...
        mkFile(storeREMOVE, "removeDirWithBackup/somedata.json");
  ...
  , "over a string": {
      topic: function(store) {
        store.set("eachFile.json", 1)
             .forEach("eachFile.json"
                     , function(item, cb) {
